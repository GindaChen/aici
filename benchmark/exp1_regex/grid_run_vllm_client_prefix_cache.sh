...
start_server() {
    local model=$1
    # Spin up the server
    sh $PROJECT_ROOT/benchmark/common/vllm-server.sh -m $model --enable-prefix-caching >> $SERVER_LOG 2>&1 &
    SERVER_PID=$!
...
for model in "${models[@]}"; do
    ...
    for prompt_length in "${prompt_lengths[@]}"; do
        for per_token_control in "${per_token_controls[@]}"; do
            ...
            python run_vllm_client.py --stream --max_tokens 128 --prompt_length $prompt_length --per_token_control $per_token_control --model $model --wandb --wandb_project "aici-token-control-enable-prefix-caching" --wandb_comment "gpu_type=$gpu_type,enable_prefix_caching=true"
            
            # Kill the server
            kill $SERVER_PID
            pkill -f 'python3 -m pyaici.vllm_server'
            # Check there is no process with "pyaici.vllm_server" running
            sleep 3
            while pgrep -f 'python3 -m pyaici.vllm_server' > /dev/null; do
                echo "Waiting for server to be killed..."
                pkill -f 'python3 -m pyaici.vllm_server'
                sleep 5
            done
            echo "Server killed."
