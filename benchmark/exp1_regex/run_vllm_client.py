...
def main_rag(args):
    ...
    prompt = args.prompt
    num_fast_forward_tokens = args.num_fast_forward_tokens
    ...
    assert stream, "RAG is not supported without streaming"

    context = []
    new_prompt = prompt + "".join(context)
    response = post_http_request(model, new_prompt, api_url, regex=regex, n=n, stream=stream, max_tokens=num_generate_tokens, sleep_time=sleep_time)
    for n_iter in range(num_iterations):
        logger.debug(f"Iteration {n_iter + 1}/{num_iterations}")
        for h in get_streaming_response(response, timer):
            first_line = None
            for i, line in enumerate(h):
                logger.debug_detail(f"Beam candidate {i}: {line!r}")
        ...
        # Concat some text to the prompt    
        for _ in range(num_fast_forward_tokens):
            context.append(" a")
    ...
    if timer.is_timer_started:
        ...
        log_result(f"Per-token latency: {timer.get_per_token_latency()} ms")
        log_result(f"Token Per Second: {1000 / timer.get_per_token_latency()} tok / s")
        log_result(f"Total time: {timer.end_time - timer.start_time} ms")


    if args.wandb:
        
        data = {
            "total_time": timer.end_time - timer.start_time,
            "first_token_latency": timer.get_first_token_latency(), 
            "per_token_latency": timer.get_per_token_latency(),
            "prompt_length": args.prompt_length,
            "per_token_control": args.per_token_control,
...
if __name__ == "__main__":
    ...
    parser.add_argument("--num_fast_forward_tokens", type=int, default=32)
    parser.add_argument("--num_generate_tokens", type=int, default=128)
